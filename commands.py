...
class Commands:
    ...
    async def _process_reply(self, user: User, reply_message: str, context: ContextTypes.DEFAULT_TYPE):
        ...
        elif button_command == CommandEnum.SEND_GUIDE_FILE:
            GUIDE_PDF_PATH = "presentation_ontaxi_driver.pdf"
            await context.bot.send_document(
                chat_id=user.chat_id,
                document=open(os.path.join(
                    self.image_folder, GUIDE_PDF_PATH), "rb")
            )
            user.reply_message_state = next_state_key
